...
export default class InventoryRepository {
  ...
  async getAll(): Promise<InventoryModel[]> {
    const result = await this.db.select().from(inventory).orderBy(inventory.createdAt);
  ...
  async getByPublicId(publicId: string): Promise<InventoryModel | null> {
    const result = await this.db
      .select()
      .from(inventory)
      .where(eq(inventory.publicId, publicId))
      .limit(1);
  ...
  async create(data: any): Promise<InventoryModel> {
    const [row] = await this.db.insert(inventory).values(data).returning();
  ...
  async delete(publicId: string): Promise<void> {
    await this.db.delete(inventory).where(eq(inventory.publicId, publicId));
  }
}
